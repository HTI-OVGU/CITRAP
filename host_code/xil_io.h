...
{
	// return readFromFPGA(Addr);
	const char *devname = XDMA_DEVICE_NAME_READ;
    ...
    u32 value = 0;
    UINTPTR rc = lseek(fpga_fd, Addr, SEEK_SET);
    if (rc != Addr) {
        perror("seek file");
        return -EIO;
    }
    rc = read(fpga_fd, (char*)&value, sizeof(u32));
    if (rc != sizeof(u32)) {
...
{
	// sendToFPGA(Addr, Value);
	const char * devname = XDMA_DEVICE_NAME_WRITE;
    int fpga_fd = open(devname, O_RDWR);
    if (fpga_fd < 0) {
        perror("unable to open device");
    }
    ...
    rc = write(fpga_fd, (char*)&Value, sizeof(u32));
    if (rc != sizeof(u32)) {
        fprintf(stderr, "%s, write underflow 0x%lx/0x%lx @ 0x%lx.\n", devname, rc, sizeof(u32), Addr);
    }
    close(fpga_fd);
}
