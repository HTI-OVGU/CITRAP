...
{
	printf("\r\n--- Entering XAxiDma_CfgInitialize() --- \r\n");
...
u32 XAxiDma_SimpleTransfer(XAxiDma *InstancePtr, UINTPTR BuffAddr, u32 Length,
			   int Direction)
{
	printf("\r\n--- Entering XAxiDma_SimpleTransfer() --- \r\n");
	...
	if (Direction == XAXIDMA_DMA_TO_DEVICE) {
		if ((Length < 1) ||
			...
			printf("XAxiDma_SimpleTransfer DMA_TO_DEVICE failed -> Exceeds Max Transfer Length\r\n");
			return XST_INVALID_PARAM;
		}
		...
		if (!InstancePtr->HasMm2S) {
			xdbg_printf(XDBG_DEBUG_ERROR, "MM2S channel is not"
				    "supported\r\n");
			printf("XAxiDma_SimpleTransfer DMA_TO_DEVICE failed -> No MM2S.\r\n");
			return XST_FAILURE;
		}
		...
		// std::cout << "Laenge: " << value << std::endl;

		if ((Length < 1) ||
		    (Length >
		     InstancePtr->RxBdRing[RingIndex].MaxTransferLen)) {
			printf("XAxiDma_SimpleTransfer DEVICE_TO_DMA failed -> Exceeds Max Transfer Length.\r\n");
			return XST_INVALID_PARAM;
		}
		...
		if (!InstancePtr->HasS2Mm) {
			xdbg_printf(XDBG_DEBUG_ERROR, "S2MM channel is not"
				    " supported\r\n");
			printf("XAxiDma_SimpleTransfer DEVICE_TO_DMA failed -> No S2MM.\r\n");
			return XST_FAILURE;
		}
